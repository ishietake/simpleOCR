...
def train(file_path, epochs, desired_error,inputhidden, inputrate  ):
    training_data = read_training_data(file_path)
    num_inputs = 12  
    num_hidden = inputhidden
    rate = inputrate
...
def maintrain(epochinput, inputrate, errorinput,neuronsinput):

    current_directory = os.path.dirname(__file__)  
    file_path = os.path.join(current_directory, 'uploadfiles', 'currentTraining.xlsx')
    epochs = int(epochinput)  # epochs typically should be an integer
    desired_error = float(errorinput)
    hiddenNeurons = int(neuronsinput)  # Assuming hidden neurons should be an integer count
    learningRate = float(inputrate)
    train(file_path, epochs, desired_error,hiddenNeurons, learningRate)

if __name__ == "__main__":
    file_path = 'datasetForANNexpanded.xlsx'  
    epochs = 100000
    errorinput = 0.0000000001
    train(file_path, epochs, errorinput)
